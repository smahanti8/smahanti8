...
🏥 **[surgical-fhir-pipeline](https://github.com/smahanti8/surgical-fhir-pipeline)** — *shipped.* Maps synthetic robotic-surgery telemetry to FHIR R4B behind a conformant REST API ($everything, CapabilityStatement, OperationOutcome), and refuses to hide what the crossing destroys: 80% of cases exchangeable, every lossy mapping logged, terminology bindings marked `PROVISIONAL` with a test that fails if anyone promotes them without the work. Ships a Provenance resource per case and a SQLite-backed governance-KPI endpoint tracking exchangeability trends across runs. Its trust-status primitive is factored out for reuse — it feeds the prior-auth-agent. Interoperability as a governance problem, treated like one.

⚖️ **[prior-auth-agent](https://github.com/smahanti8/prior-auth-agent)** — *in progress.* A LangGraph agent drafting prior-authorization in progress: FHIR bundle + CPT → RAG policy retrieval → per-criterion evidence extraction → determination. One hard rule: it never drafts a denial (schema emits only approve / insufficient_evidence; humans make adverse calls). A bilateral citation gate rejects any “met” criterion missing both a policy quote and a FHIR citation. A deterministic confidence gate — not the model — decides auto vs. human review. Backed by policy-as-code criteria beside RAG, a hash-chained audit log, a 4-dimension golden eval suite with zero-cost cassette CI, and a Terraform PHI-boundary reference (Fargate/KMS/S3 object-lock). Next: populate the eval baseline with a live run, calibrate the 0.85 threshold, fix vacuous auto-approve on empty criteria.
